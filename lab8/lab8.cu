...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  if (xPos < dim) {
    ...
    // Iterate through each column index that corresponds to this row
    for (int j = 0; j < matRows[xPos]; ++j) {
      int colIndex = matCols[matColStart[j] + xPos];
      sum += matData[matColStart[j] + xPos] * vec[colIndex];
    }
    out[matRowPerm[xPos]] = sum;
  }
}
...
static void spmvJDS(float *out, int *matColStart, int *matCols,
                    ...
                    float *vec, int dim) {
  int blockSize = 128; // Set block size to 128
