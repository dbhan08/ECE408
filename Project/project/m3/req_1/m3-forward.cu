...
__global__ void matrixMultiplyShared(const float *A, const float *B, float *C,
    ...
    // Shared memory for tiles (in half precision for Tensor Cores)
    ...
    // Tile width is same as wmma_M , wmma_N ,wmma_K which is 16 from the article
    ...
    for (int tileId = 0; tileId < tiles; tileId++) {
        // Load A tile from global memory into shared memory, converting to half
        ...
        // Load B tile from global memory into shared memory, converting to half
